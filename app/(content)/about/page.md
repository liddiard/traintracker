...
Enterprising [people have figured out](https://suddenlygreg.com/blog/2023-11-02-amtrak-api/) how to decrypt this data. For the record, if you know how the web works, you’ll immediately understand that there’s nothing nefarious going on here. It’s not really possible to obscure something when Amtrak also makes the decryption keys public.
...
TrainTracker follows responsible caching practices to ensure no traffic burden of significance is imposed on these upstream data providers.
...
On the other hand, apps like Google Maps, [CityMapper](https://citymapper.com/), and [Transit](https://transitapp.com/) are pretty great for planning city and metro area trips on transit like commuter rail and subways. These apps need to solve routing problems, handle multiple forms of transit, and time transfers. There’s a lot more complexity, and it’s already a pretty well-served space.

Simply put, TrainTracker excels in longer-distance rail travel and isn’t looking to compete on local transit.

As for geographic scope, TrainTracker is focused on North America because unlike many European and Asian markets, it doesn’t have particularly great offical transit apps doing the sort of thing TrainTracker does. Candidly, it’s also because that’s where I live. I’m open to expanding TrainTracker to other regions, but it’s not a current priority.

### Why are TrainTracker’s train locations wrong sometimes?

TrainTracker uses a combination of last-reported GPS location and station ETAs to extrapolate trains’ real-time positions. By necessity, this is somewhat optimistic. For example, we may have last received a GPS position from a train 5 minutes ago. Based on its estimated arrival time at the next station, we _assume_ it’s continued to move along the track and show an estimated current position. It’s an informed guess, but ultimately it is a guess, so sometimes it’s wrong.

When a train is selected on the map, TrainTracker shows a rotating crosshair that indicates that train’s last-reported position. You can compare this with the train’s arrow icon which shows extrapolated position.
